...
namespace Crawl_console
{
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("Xuất file Product.xlsx thành công!");
    ...
    public class Crawl_DanhMuc_href
    {
        ...
        public List<DanhMuc> crawl()
        {
            ...
            using(WebClient web = new WebClient())
            {
                ...
                {
                    Console.WriteLine("Không có dữ liêu!");
                }
                return ListDanhMuc;
            }
        }
        ...
        public int GetSizePage ()
        {
            ...
            using (WebClient web = new WebClient())
            {
                ...
                if(elements != null)
                {
                    ...
                    {
                        Console.WriteLine($"Unable to parse '{sizeStr}'");
                    }
                }
                else
                {
                ...
                Console.WriteLine($"Size of Page {_danhmuc.title}: {sizePage}");
            }
            return sizePage;
        }
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        Console.WriteLine($"{_danhmuc.title}: {pageSize}");
                    }
                    else
                    {
                        Console.WriteLine($"Lỗi: {response.StatusCode} - {response.ReasonPhrase}");
        ...
        public async Task<Product> CrawlProductWithApi(PreviewProduct previewProduct)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"Lỗi: {response.StatusCode} - {response.ReasonPhrase} - product id: {previewProduct.ProductId}");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Lỗi: {ex.Message} - product id: {previewProduct.ProductId}");
                }
                return product;
            }
        }
        public async Task<Shop> CrawlDataShopWithApi( Shop shop)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"Lỗi: {response.StatusCode} - {response.ReasonPhrase} - product id: {shop.Id}");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Lỗi: {ex.Message} - product id: {shop.Id}");
                }
                return shop;
            }
        }
